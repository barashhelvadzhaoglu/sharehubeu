...
# Hedef dosyalar listesi (Aynı liste)
target_files = [
...
def apply_publish_fix():
    print(f"--- YAYINLAMA OPERASYONU BAŞLADI ---")
    updated_count = 0
    ...
    for file_path in target_files:
        if os.path.exists(file_path):
            ...
            # Eğer hiç draft satırı yoksa, false olarak ekle (yayınlamak için)
            if not draft_found:
                for i, line in enumerate(lines):
                    if line.strip().startswith("date:"):
                        lines.insert(i + 1, "draft: false\n")
                        modified = True
                        break
            ...
            if modified:
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.writelines(lines)
                print(f"[YAYINDA] {file_path}")
                updated_count += 1
        else:
            print(f"[HATA] Dosya bulunamadı: {file_path}")

    print(f"\nİşlem bitti. Toplam {updated_count} dosya draft:false yapıldı.")
